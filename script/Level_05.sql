...
SELECT
    r.month,
    ROUND(
        ((r.total_revenue - r.prev_revenue) / NULLIF(r.prev_revenue,0)) * 100, 2
    ) AS revenue_growth_pct,
    ROUND(
        ((m.total_marketing_spend - m.prev_spend) / NULLIF(m.prev_spend,0)) * 100, 2
